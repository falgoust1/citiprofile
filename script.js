const { DeckGL, ScatterplotLayer, ScreenGridLayer, GeoJsonLayer, GridLayer, HeatmapLayer, HexagonLayer,Slider, DataFilterExtension, FlyToInterpolator } = deck;
...
toggle.addEventListener('change', (e) => {
  if (e.target.checked) {
    ...
    function updateLayer() {
      ...
      let filteredPoints = points;  // par défaut, tous les points

        if (selectedQuartierPolygon) {
          filteredPoints = points.filter(point => {
            const pointGeoJSON = turf.point([point.lon, point.lat]);
            return turf.booleanPointInPolygon(pointGeoJSON, selectedQuartierPolygon);
          });
        }
      ...
      if (scatterSelected) {
        ...
        newLayer = new GridLayer({
          ...
          extruded: true,
          getPosition: d => [d.lon, d.lat],
          getElevationValue: filteredPoints => filteredPoints.length,
      ...
      deckgl.setProps({ 
       ...
       const quartiersLayer = new deck.GeoJsonLayer({
        ...
        getLineColor: [37, 211, 102],   //rgb(46, 46, 46) => en [R, G, B]
        ...
        /* Interactions */
        pickable: true,         // rend la couche "cliquable/hoverable"
        autoHighlight: true,    // active un effet de survol automatique
        highlightColor: [102, 205, 170, 70], // couleur survol: blanc semi-transparent
        
        /* Hover sur le quartier => mise à jour du #quartier-nom */
        onHover: info => {
          if (info.object) {
        ...
        onClick: info => {
          if (info.object) {
            ...
            selectedQuartierPolygon = info.object.geometry; // Sauvegarde du polygone sélectionné
        
            // Zoom sur le quartier sélectionné
            ...
            updateLayer();  // ⚠️ Mise à jour des layers après sélection du quartier
          }
        }
